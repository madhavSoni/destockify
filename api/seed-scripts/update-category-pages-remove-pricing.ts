...
// Category page updates - removing pricing, keeping directory focus
...
async function main() {
  console.log('🚀 Starting category page updates (removing pricing)...\n');
  ...
  try {
    ...
    for (const update of categoryUpdates) {
      try {
        ...
        if (!existing) {
          console.log(`⚠️  Skipping ${update.slug}: page not found`);
          skippedCount++;
          continue;
        }
        ...
        await prisma.categoryPage.update({
          where: { slug: update.slug },
          data: {
            heroText: update.heroText,
            featuredSuppliersText: update.featuredSuppliersText,
            centeredValueText: update.centeredValueText,
          },
        });
