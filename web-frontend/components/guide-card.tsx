...
export function GuideCard({ guide }: { guide: GuideSummary }) {
  return (
    <Link
      ...
      className="group flex h-full flex-col justify-between rounded-md border border-black/10 bg-white p-6 shadow-sm transition hover:shadow-md"
    >
      <div>
        <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-blue-600">
          Insight
        </div>
        <h3 className="mt-3 text-xl font-semibold text-black group-hover:text-blue-600">{guide.title}</h3>
        {guide.intro && <p className="mt-3 text-sm text-black/70">{guide.intro}</p>}
      </div>
      <div className="mt-6 flex items-center justify-between border-t border-black/5 pt-4 text-sm font-medium text-black">
        Read guide
