...
export function generateWebsiteSchema(): WithContext<WebSite> {
  return {
    ...
    potentialAction: ({
      ...
      target: {
        '@type': 'EntryPoint',
        urlTemplate: 'https://destockify.com/suppliers?search={search_term_string}'
      },
...
export function generateSupplierSchema(
  ...
  const schema: WithContext<LocalBusiness> = {
    ...
    '@type': 'LocalBusiness',
    name: supplier.name,
    url: `https://destockify.com/suppliers/${supplier.slug}`,
...
export function generateArticleSchema(guide: any): WithContext<Article> {
  return {
    ...
    url: `https://destockify.com/guides/${guide.slug}`,
...
export function generateBreadcrumbSchema(items: Array<{ name: string; url: string }>): WithContext<BreadcrumbList> {
  return {
    ...
    itemListElement: items.map((item, index) => ({
      ...
      position: index + 1,
      name: item.name,
      item: `https://destockify.com${item.url}`
    }))
  };
}
