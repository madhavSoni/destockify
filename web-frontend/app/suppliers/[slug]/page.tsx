  ...
  return (
    <section className="rounded-3xl border-2 border-slate-900/80 bg-white overflow-hidden">
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="flex items-start gap-6">
          ...
          <div className="flex-1 min-w-0">
            ...
            {supplier.shortDescription && (
              <p className="font-medium text-base text-slate-600">{supplier.shortDescription}</p>
            )}
            
            {/* Region & Badges */}
            <div className="mt-4 flex flex-wrap items-center gap-2">
              {supplier.region && (
                <span className="inline-flex items-center gap-1.5 rounded-xl bg-blue-100 border-2 border-blue-200 px-4 py-2 text-sm font-bold text-blue-700">
              ...
              {supplier.badges?.map((badge, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 rounded-xl bg-green-100 border-2 border-green-200 px-4 py-2 text-sm font-bold text-green-700">
        ...
        {/* Rating & Actions */}
        <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t-2 border-slate-200 pt-6">
          <div className="flex items-center gap-3">
            <RatingStars rating={rating} />
            <div className="flex items-center gap-2">
              <span className="font-black text-2xl text-slate-900">{rating.toFixed(1)}</span>
    ...
    <section className="rounded-3xl border-2 border-slate-900/80 bg-white p-6 sm:p-8">
      ...
      <div className="space-y-6">
        {supplier.description && (
          <p className="font-medium text-base text-slate-700 leading-relaxed">{supplier.description}</p>
        )}
        ...
        {supplier.specialties && supplier.specialties.length > 0 && (
          <div>
            <h3 className="font-black text-xl text-slate-900 mb-3">Specialties</h3>
            <div className="flex flex-wrap gap-2">
              {supplier.specialties.map((specialty, i) => (
                <span key={i} className="rounded-xl bg-purple-100 border-2 border-purple-200 px-4 py-2 text-sm font-bold text-purple-700">
                  {specialty}
                </span>
              ))}
            </div>
        ...
        {supplier.certifications && supplier.certifications.length > 0 && (
          <div>
            <h3 className="font-black text-xl text-slate-900 mb-3">Certifications</h3>
            <div className="flex flex-wrap gap-2">
              {supplier.certifications.map((cert, i) => (
                <span key={i} className="inline-flex items-center gap-1.5 rounded-xl bg-amber-100 border-2 border-amber-200 px-4 py-2 text-sm font-bold text-amber-700">
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        ...
        {(supplier.logisticsNotes || supplier.pricingNotes) && (
          <div className="space-y-4">
            {supplier.logisticsNotes && (
              <div className="rounded-2xl border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-blue-100 p-5">
                <div className="flex items-center gap-2 mb-2">
                  <svg className="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                  </svg>
                  <div className="font-black text-sm text-blue-900">Logistics</div>
                </div>
                <p className="font-medium text-sm text-blue-800">{supplier.logisticsNotes}</p>
              </div>
            )}
            {supplier.pricingNotes && (
              <div className="rounded-2xl border-2 border-emerald-200 bg-gradient-to-r from-emerald-50 to-emerald-100 p-5">
                <div className="flex items-center gap-2 mb-2">
                  <svg className="w-5 h-5 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <div className="font-black text-sm text-emerald-900">Pricing</div>
                </div>
                <p className="font-medium text-sm text-emerald-800">{supplier.pricingNotes}</p>
              </div>
            )}
          </div>
        )}
